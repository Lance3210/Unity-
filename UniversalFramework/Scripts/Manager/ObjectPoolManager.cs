...
public class ObjectPoolManager : SingletonManagerMonoBase<ObjectPoolManager> {
	private Dictionary<string, Shelf> poolDic = new Dictionary<string, Shelf>(); // Shelf 封装存储架
	...
	public GameObject GetObject(GameObject prefab) {
		GameObject obj;
		if (poolDic.ContainsKey(prefab.name) && poolDic[prefab.name].shelfQueue.Count > 0)//池中有 Key 且该存储架不为空则取出
		{
			obj = poolDic[prefab.name].PopObj(ref prefab);
		}
		else {
	...
	public void PushObject(GameObject targetObj) {
		...
		if (poolDic.ContainsKey(targetObj.name))                //池中有 Key 则放回对应存储架
		{
			poolDic[targetObj.name].PushObj(ref targetObj);
		}
		else {
